...
def startedSpeaking(name):
    global isSpeaking
    isSpeaking = True

    
def finishedSpeaking(name, completed):
    global isSpeaking
    isSpeaking = False
    engine.endLoop()


engine.connect('started-utterance', startedSpeaking)
engine.connect('finished-utterance', finishedSpeaking)
...
#            if last_speech + squat_cooldown < ts:
            if not isSpeaking:
                if last_speech + squat_cooldown < ts:
                    # Good form
                    if pred == 2:
                    ...
                    if pred == 1:
                        last_speech = ts
                        #engine.endLoop()
                        engine.say("Bad squat correct your form")
                        engine.startLoop()
                        #engine.runAndWait()
